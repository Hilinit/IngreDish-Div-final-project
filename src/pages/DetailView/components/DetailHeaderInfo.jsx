...
const DetailHeaderInfo = ({ currentItem, isBlog, isActive, handleToggle }) => {
  return (
    <div className="bg-white dark:bg-neutral-900 rounded-3xl p-5 sm:p-8 shadow-sm border border-slate-100/80 dark:border-neutral-800 transition-colors">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 dark:border-neutral-800 pb-6 mb-6">
        <div className="flex flex-wrap items-center gap-3 sm:gap-6 text-xs sm:text-sm text-slate-600 dark:text-gray-300 font-medium">
        ...
        <button
          ...
          className={`p-3 rounded-2xl font-semibold text-sm transition border cursor-pointer ${
            isActive ? "bg-orange-50 dark:bg-orange-950/40 text-[#C2410C] dark:text-orange-400 border-orange-200 dark:border-orange-900/50" : "bg-slate-50 hover:bg-slate-100 dark:bg-neutral-800 dark:hover:bg-neutral-700 text-slate-700 dark:text-gray-300 border-slate-200 dark:border-neutral-700"}`}
        >
          {isBlog ? ( 
            isActive ? <FaBookmark className="text-xl text-[#C2410C] dark:text-orange-400" /> : <FaRegBookmark className="text-xl"/>
          ) : (
            isActive ? <FaHeart className="text-xl text-red-500" /> : <FaRegHeart className="text-xl" />
          )}
        </button>
      </div>
