...
const EditProfileModal = ({ isOpen, onClose, user, newName, setNewName, newAvatar, handleImageChange, handleSaveProfile }) => {
  ...
  const formFields = [
    { id: "name", label: "Ad Soyad", type: "text", value: newName, onChange: (e) => setNewName(e.target.value), required: true, placeholder: "Adınızı daxil edin", disabled: false},
    { id: "email", label: "E-poçt (Gmail)", type: "email", value: user?.email || "", disabled: true, placeholder: ""}
  ];
        ...
        <form onSubmit={handleSaveProfile} className="space-y-6">
          <div className="flex flex-col items-center justify-center space-y-3">
            <div className="relative group">
              <img loading="lazy" src={newAvatar || "https://i.pinimg.com/736x/4d/68/ec/4d68eca5e68d4141861e00aa728a73fc.jpg"} alt="Profil" className="w-28 h-28 rounded-3xl object-cover border-4 border-slate-100 dark:border-neutral-800 shadow-md" />
          ...
          {formFields.map((field) => (
            <div key={field.id} className="space-y-1.5">
              ...
              <input  type={field.type}  value={field.value}  onChange={field.onChange}  required={field.required}  disabled={field.disabled} placeholder={field.placeholder} className={`w-full border rounded-xl px-4 py-2.5 text-xs transition ${ field.disabled 
                    ? "bg-slate-100 dark:bg-neutral-800/50 border-slate-200 dark:border-neutral-700 text-slate-400 dark:text-slate-500 cursor-not-allowed" 
                    : "bg-slate-50 dark:bg-neutral-800 border-slate-200 dark:border-neutral-700 text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-[#C2410C]/20 dark:focus:ring-orange-500/20 focus:border-[#C2410C] dark:focus:border-orange-500"
                }`} 
              />
            </div>
          ))}
